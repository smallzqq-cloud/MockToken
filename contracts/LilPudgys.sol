...
import "./others/Erc721/LilPudgys/ERC721.sol";
...
contract LilPudgys is ERC721, Ownable, WithdrawFairly, ReentrancyGuard {
    ...
    uint256 public constant MAX_AUCTION =
        MAX_ELEMENTS - MAX_CLAIM - MAX_RESERVE;
    ...
    function mintById(address addr, uint256 tokenId) public {
        _mint(addr, tokenId);
    }
    ...
    function mint(uint256 _count) public payable saleIsOpen nonReentrant {
        ...
        for (uint256 i = 0; i < _count; i++) {
            _mintToken(
                _msgSender(),
                MAX_CLAIM + MAX_RESERVE + _tokenIdTrackerAuction
            );
    ...
    function claim(uint256[] memory _tokensId) public claimIsOpen {
        ...
        for (uint256 i = 0; i < _tokensId.length; i++) {
            require(
                canClaim(_tokensId[i]) &&
                    pudgyPenguins.ownerOf(_tokensId[i]) == _msgSender(),
                "Bad owner!"
            );
    ...
    function reserve(uint256 _count) public onlyOwner {
        require(
            _tokenIdTrackerReserve + _count <= MAX_RESERVE,
            "Exceeded giveaways."
        );
        for (uint256 i = 0; i < _count; i++) {
    ...
    function getMintPrice(uint256 _timestamp) public view returns (uint256) {
        ...
        uint256 currentPrice = dutch.startPrice -
            ((((duration * 100000) / dutch.duration) *
                (dutch.startPrice - dutch.endPrice)) / 100000);
