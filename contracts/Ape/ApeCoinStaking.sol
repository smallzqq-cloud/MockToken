...
contract ApeCoinStaking is Ownable {
     ...
     * @member lastRewardedTimestampHour 表示最后一次奖励分配发生的小时级别时间戳。这个变量的目的是避免重复分配奖励
     * @member lastRewardsRangeIndex 表示最后一次奖励分配所使用的时间范围的索引。具体来说，合约中维护了一个时间范围数组 timeRanges，每个时间范围对应着一个奖励分配周期。这个变量用来记录上一次分配奖励时使用的时间范围，以便下一次分配奖励时可以从这个时间范围开始计算
     * @member stakedAmount 表示当前抵押在这个池子中的代币数量。这个变量会在用户抵押或赎回代币时进行更新
     * @member accumulatedRewardsPerShare 表示每个代币所应获得的奖励数量。
     */ 
    struct Pool {
     ...
     * @member startTimestampHour Staking 时间段的开始时间
     * @member endTimestampHour  结束时间
     * @member rewardsPerHour 量则是数值，表示在这个时间段内参与 Staking 的用户可以获得的奖励
     * @member capPerPosition 每个用户参与 Staking 的代币数量上限
     */ 
    struct TimeRange {
     ...
     * @member stakedAmount 表示这个流动性提供者质押的代币数量。
     * @member rewardsDebt 表示这个流动性提供者已经领取的奖励金额
     */
    struct Position {
        uint256 stakedAmount;
        int256 rewardsDebt;
    ...
    /** 
     * @dev UI focused payload
     * @member poolId 表示这个质押对应的奖励池的编号，ApeCoinStaking合约中可以有多个奖励池，每个奖励池对应不同的代币和奖励。
     * @member tokenId 表示这个质押所对应的代币的编号，ApeCoinStaking合约中可以接受多种代币的质押。
     * @member deposited 表示这个质押所质押的代币数量
     * @member unclaimed 表示这个质押当前尚未领取的奖励数量。
     * @member rewards24hr 表示这个质押在最近的24小时内已经获得的奖励数量。
     * @member pair 表示这个质押对应的交易对信息，包括代币A和代币B的地址，以及对应的汇率等信息。
     */ 
    ...
    function depositApeCoin(uint256 _amount, address _recipient) public {
        if (_amount < MIN_DEPOSIT) revert DepositMoreThanOneAPE(); //至少需要质押1个 ape
        updatePool(APECOIN_POOL_ID);//更新池子状态

        Position storage position = addressPosition[_recipient]; //获取接受者的质押代币状况， 
        _deposit(APECOIN_POOL_ID, position, _amount); //质押

        apeCoin.transferFrom(msg.sender, address(this), _amount); //转账
     ...
     * @param _poolId Available pool values 0-3
     * @param _from Whole hour timestamp representation 上次挖矿分配奖励的时间
     * @param _to Whole hour timestamp representation  最近的小时时间戳
     */
    function rewardsBy(uint256 _poolId, uint256 _from, uint256 _to) public view returns (uint256, uint256) {
        Pool memory pool = pools[_poolId];

        uint256 currentIndex = pool.lastRewardsRangeIndex; //上次分配奖励的时间索引
        if(_to < pool.timeRanges[0].startTimestampHour) return (0, currentIndex); //如果最近的小时时间戳小于第一个挖矿阶段的开始时间就返回 0 和上次分配奖励的时间
        //如果上次分配奖励的时间大于上一次挖矿的结束时间并且最近的小时时间戳大于最近一次挖矿的结束时间，就一直循环 currentIndex + 1，直到找到有交集的时间段。
    ...
    function updatePool(uint256 _poolId) public {
        ...
        if (block.timestamp < pool.timeRanges[0].startTimestampHour) return; //时间要大于第一次 质押的开始时间
        if (block.timestamp <= pool.lastRewardedTimestampHour + SECONDS_PER_HOUR) return; //时间至少需要超过上一次分配奖励一小时

        uint48 lastTimestampHour = pool.timeRanges[pool.timeRanges.length-1].endTimestampHour; //获取最新的一次挖矿的结束时间
        uint48 previousTimestampHour = getPreviousTimestampHour().toUint48(); //获取最近的一个小时时间戳
        //如果池子里没有币，就判断 上一次挖矿结束的时间 和 最近一个小时的时间，哪个小，就把哪个赋值给上一次挖矿奖励的时间
        ...
        //获取你要质押代币的这个时候所处的时间段的奖励以及时间范围ID
